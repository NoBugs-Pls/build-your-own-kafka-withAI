...
public class ZookeeperClient implements Watcher {
    ...
    /** 连接等待上限：ZooKeeper 未启动时快速失败，避免调用方（如可视化界面）长时间挂起。 */
    private static final long CONNECT_WAIT_TIMEOUT_MS = 3000L;
    ...
    public void connect() throws IOException, InterruptedException {
        ...
        if (!connectedSignal.await(CONNECT_WAIT_TIMEOUT_MS, TimeUnit.MILLISECONDS)) {
            try {
                zooKeeper.close();
            } catch (Exception ignored) {
                // 连接失败后的清理以尽力而为
            ...
            zooKeeper = null;
            throw new IOException("Timed out connecting to ZooKeeper at " + getConnectString());
        }
    ...
    /** 当前会话是否仍然可用；用于 broker 的注册自愈逻辑判断是否需要重连。 */
    public boolean isConnected() {
        ZooKeeper client = zooKeeper;
        return client != null && client.getState().isAlive() && connectedSignal.getCount() == 0;
    }
