...
9. [已知限制与第三阶段计划](#9-已知限制与第三阶段计划)
10. [开发阶段与验证记录](#10-开发阶段与验证记录)
...
项目按「先能跑通 → 再修正确性 → 再做可靠性」推进，每一阶段都以**可复现的实测**收尾。
测试代码已入库：存储层单测在 `src/test/java/`，线上脚本在 `tools/verify/`。

### 10.1 阶段一：可用性与持久化

| 主题 | 关键改动 | 实测证据 |
| --- | --- | --- |
| 启动与接管 | 控制台用 `ProcessHandle` 扫描并接管 ZooKeeper 与 broker | 控制台重启后 4 个进程仍可控 |
| 停止集群 | 限时关闭 ZK + hook 内 `halt(0)`；SIGTERM → 4s 后 `destroyForcibly` + 兜底清扫 | 「停止全部」从无效变为稳定生效 |
| Controller 与副本 | 建 topic 用 ZK 实时 `/brokers` 选副本；leader/follower 缓存按需刷新 | 分配稳定为 `(0,1,[2]),(1,2,[3]),(2,3,[1])` |
| **持久化五项** | ①崩溃恢复（脏尾截断）②索引重建 ③跨段读取 ④稀疏索引（8B/条，每 4KB）⑤段轮转与索引归属 | 120×10KB → 2 段；索引占比 **0.080%**；写脏尾后重启自动截断，offset 不变 |
| 链路层修复 | `REPLICATE` 容量 17→19、转发 9→11；读缓冲 1KB→64KB；客户端按长度精确读取；超时齐备 | 复制从「静默失效」→ 副本字节数一致；1MB 拉取不再 `BufferUnderflowException` |

### 10.2 阶段二：保留策略与拉取式复制

| 主题 | 关键改动 | 实测证据 |
| --- | --- | --- |
| ① 保留策略 | `Partition.cleanup()` 按大小/时间整段删除；`logStartOffset` + `OffsetOutOfRange`；保活线程每 5 秒触发 | 82KB / 上限 64KB → 删 2 段（6→4）；`fetch(0)` 报 `... log start offset 14` |
| ② pull 复制 + 追赶 | `REPLICA_FETCH` 协议、`replica-fetcher` 线程、长轮询、长连接、`truncateBefore` 起点对齐；删除 push 复制 | 杀掉 follower → 写 20 条 → 重启 → **2 秒追平**（连跑 6+ 轮） |
| 可靠性修复 | 副本分配排序（可复现）；保活线程刷新「最后在线时间」修宽限期竞态；节流 sleep 100/50ms → 1ms；日志降噪 | 6 轮追赶全部通过；空闲日志 3.2 → **0.2 条/秒** |
| 控制台集群隔离 | 进程扫描按 `zkPort` 限定，避免把「另一套集群的同 id broker」当成重复进程强杀 | 跑第二套集群（同 id、端口 9191/9192）后，主集群 PID 不变、3 副本全在 |

### 10.3 如何复现这些验证

```bash
# 1) 存储层单测（不需要集群；用几 KB 的段与很小的保留上限就能跑完）
mvn clean package -DskipTests
java -Dsimplekafka.segment.bytes=65536 -Dsimplekafka.retention.bytes=131072 \
     -cp target/classes:target/test-classes com.simplekafka.broker.PartitionSelfTest
# 预期：===> ALL TESTS PASSED
#   覆盖：脏尾三种形态、索引删除自愈、复制幂等、保留策略（字节/时间）、活跃段保护、起点对齐

# 2) 保留策略线上验证：脚本会自建一套隔离的小集群（ZK 2281 + broker 9191/9192，数据在 /tmp）
python3 tools/verify/live_retention.py
# 预期：===> 保留策略线上验证通过

# 3) pull 复制追赶验证：需要控制台(8080) + 3 个 broker 已启动，topic 名传新的以便从 0 开始
python3 tools/verify/live_catchup.py my-topic-1
# 预期：===> pull 复制追赶验证通过（含“重启后 2 秒追平”）
```
